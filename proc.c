...
static void assert_prio(struct proc * p, int priority);
static void promote_prio(void);
#endif // CS333_P4
...
allocproc(void)
{
  ...
  acquire(&ptable.lock);
  int found = 0;
  //find the next UNUSED process using the UNUSED list instead of this nonsense
#ifdef CS333_P3
...
#endif //CS333_P3 altered version
...
yield(void)
{
  ...
  //TODO possible panic here? changed from list to ready above but did not fix problem
  sched();
  release(&ptable.lock);
}
//original version
...
{
  uint end = ticks - p->start_ticks; 
  int sec = end / 1000;
  int milli = end % 1000; 
  //PID Name Elapsed State Size
  cprintf("\n%d\t%s\t     %d.%d%d%d\t%s\t%d\t", p->pid, p->name, sec, milli/100, milli%100/10, milli%10, state_string, p->sz);
  return;
}
#endif
